...
class LLM_Agent_GT(Agent):
    def __init__(self, args, config_env, agent_id, device) -> None:
        self.args = args
    ...
    def mapping(self, observations: Observations):
        ...
        if self.args.use_sam:
            ...
            obs = self._preprocess_obs_rednet(state, semantic, cur_mapping) 
    ...
    def remove_small_points(self, local_ob_map, image, threshold_point, pose):
        ...
        img_label, num = measure.label(image, connectivity=2, return_num=True) # Output all connected fields in the binary image
